...
import time
import numpy as np
from utils import raw_data, get_num_parameters, save_model, repackage_hidden, generate_batch, generate_batch2
...
def run_epoch(model, data, is_train=False, lr=1.0):
    ...
    for batch_idx, (input, target) in enumerate(generate_batch2(data, model.batch_size, model.num_steps, num_workers)):
        # model.zero_grad()
        # print("\nbatch_idx#",batch_idx)
        ...
        if is_train:
            ...
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 0.25)    # To prevent the exploding gradient problem.
