...
def generate_batch_(raw_data, batch_size, num_steps, num_workers):
    """
    This function iterates on the raw data and generates batch_size pointers,
    which allows minibatch iteration along these pointers.
    Args:
    ...
    for i in range(epoch_size):
        ...
        # print(x.shape)
        # y = data[j*batch_size:(j+1)*batch_size, i*num_steps+1:(i+1)*num_steps+1]
        # y = data[(i+1)*batch_size*num_steps:(i+2)*batch_size*num_steps]
        # y = data[(i+1)*batch_size:(i+3)*batch_size]
        # y = data[i*batch_size*num_steps+eff_batch_size:(i+1)*batch_size*num_steps+eff_batch_size]
        y = data[i*batch_size*num_steps+batch_size:(i+1)*batch_size*num_steps+batch_size]
        y = data[i*batch_size*num_steps+1:(i+1)*batch_size*num_steps+1]
        # print("y", y)
